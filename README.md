...
6. [Docker Compose](#docker-compose-1)
7. [Makefile]()
...
Volume are used for persisting data generated by and used by Docker containers. Volume's contents exist outside the lifecycle of a given container.
...
Specifies which volume driver should be used.

`driver: local` : indicates that Docker has to create a volume that will be stored on the local filesystem of the Docker host. It's directly bind to the filesystem of the machine where Docker is running, so the data is stored even if the container is removed.

#### `driver_opts`

Specifies a list of options to pass to the driver, it is driver-dependent.

`type: none` : defines the type of filesystem or bind that the volume uses. `none` indicates that Docker don't try to manage the volume's filesystem.

`o: bind` : specifies how the volume has to be mounted. `bind` indicates that Docker has to bind an existing directory on the host directly into the container, rather than creating a dedicated Docker-managed volume.

`device: <...>` : specifies the path to the directory on the host that should be mounted in the container.
...
For each service, we indicate which volume has to be used in the container and the corresponding path inside the container where that volume will be mounted.
...
<summary><strong>Volumes</strong></summary>
 ...
 - [Docker compose and volumes](https://docs.docker.com/compose/compose-file/07-volumes/)
</details>
